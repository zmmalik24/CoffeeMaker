...
public class APIInventoryController extends APIController {
     ...
     * @return response to the request
     */
    @PutMapping ( BASE_PATH + "/inventory/{name}/{amount}" )
    public List<Ingredient> updateInventory ( @PathVariable final String name, @PathVariable final int amount ) {
        final Inventory inventoryCurrent = service.getInventory();
        
        Ingredient newIngredient = new Ingredient(name, amount);
        
        if (!(name.equals("initial"))) {
        	//inventoryCurrent.addIngredient(newIngredient);
        	inventoryCurrent.updateAmount(newIngredient);
        }
        //inventoryCurrent.addIngredient(newIngredient); 
        service.save( inventoryCurrent );
        return inventoryCurrent.getIngredients();
    }
}
